...
class _AdminSettingsScreenState extends State<AdminSettingsScreen>
  ...
  bool _highPriorityAlerts     = true;
  bool _weeklyDigest           = false;
  bool _autoAssign             = false;
  bool _twoFactor              = false;
  int  _resolutionDeadlineHours = 48;
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      if (res.statusCode == 200 && mounted) {
        ...
        setState(() {
          _emailNotifications      = s['email_notifications']      == true;
          _pushNotifications       = s['push_notifications']       == true;
          _highPriorityAlerts      = s['high_priority_alerts']     == true;
          _weeklyDigest            = s['weekly_digest']            == true;
          _autoAssign              = s['auto_assign']              == true;
          _twoFactor               = s['two_factor']               == true;
          _resolutionDeadlineHours = (s['resolution_deadline_hours'] ?? 48) as int;
          _isLoading               = false;
        });
      } else {
  ...
  Future<void> _loadStats() async {
    try {
      ...
      if (res.statusCode == 200 && mounted) {
        final s = jsonDecode(res.body) as Map<String, dynamic>;
        setState(() {
          _openCount     = (s['open']     ?? 0) as int;
          _resolvedCount = (s['resolved'] ?? 0) as int;
          _agentCount    = (s['agents']   ?? 0) as int;
        });
      }
    } catch (_) {}
  }
    ...
    try {
      final res = await http.put(
        ...
        body: jsonEncode({
          ...
          'high_priority_alerts':     _highPriorityAlerts,
          'weekly_digest':            _weeklyDigest,
          'auto_assign':              _autoAssign,
          'two_factor':               _twoFactor,
      ...
      if (mounted) _snack('Error connecting to server', isError: true);
    } finally {
  ...
  Future<void> _changePassword({
    ...
    try {
      final res = await http.post(
        ...
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'current_password': current, 'new_password': newPw}),
      );
      if (mounted) {
        _snack(
          res.statusCode == 200
  ...
  Future<void> _exportComplaints() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (mounted) _snack('Export failed', isError: true);
      }
    } catch (_) {
      if (mounted) _snack('Error connecting to server', isError: true);
    }
  ...
  // ── Reset portal data ──────────────────────────────────────────────────────
  Future<void> _resetPortalData() async {
    try {
      ...
      if (mounted) {
        _snack(
          res.statusCode == 200
              ? 'Portal data has been reset'
              : 'Failed to reset portal data',
          isError: res.statusCode != 200,
        );
      }
    } catch (_) {
  ...
  void _snack(String msg, {bool isError = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: isError ? kRed : kDeepViolet,
              ...
              child: Column(
                ...
                children: [
                  ...
                  _PwField(
                    ...
                    obscure: _curObscure,
                    onToggle: () =>
                        setSheetState(() => _curObscure = !_curObscure),
                  ),
                  ...
                  _PwField(
                    ...
                    obscure: _newObscure,
                    onToggle: () =>
                        setSheetState(() => _newObscure = !_newObscure),
                  ),
                  const SizedBox(height: 10),
                  _PwField(
                    controller: _conPwCtrl,
                    hint: 'Confirm new password',
                    obscure: _conObscure,
                    onToggle: () =>
                        setSheetState(() => _conObscure = !_conObscure),
                  ),
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            if (np.length < 6) {
                              _snack('Minimum 6 characters required', isError: true);
                              return;
                            }
                            Navigator.pop(ctx);
  ...
  void _openDeadlinePicker() {
    final ctrl = TextEditingController(
      text: _resolutionDeadlineHours.toString(),
    );
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
                ...
                children: [
                  ...
                  const Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 2),
                      Text(
                        'Set default SLA hours per complaint',
                        style: TextStyle(fontSize: 12, color: kInkLight),
                      ),
                    ],
                  ),
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        if (val == null || val <= 0) {
                          _snack('Enter a valid number of hours', isError: true);
                          return;
                        }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SLIVER HEADER  ← professional redesign
  // ══════════════════════════════════════════════════════════════════════════
      ...
      child: Column(
        children: [
          _ToggleRow(
            ...
            subtitle: 'Complaint updates via email',
          ...
          _ToggleRow(
            ...
            subtitle: 'Real-time alerts on new complaints',
          ...
          _ToggleRow(
            ...
            subtitle: 'Instant alert for critical issues',
            value: _highPriorityAlerts,
            onChanged: (v) {
              setState(() => _highPriorityAlerts = v);
              _saveSettings();
            },
          ),
          const _RowDivider(),
      ...
      child: Column(
        children: [
          _ToggleRow(
            ...
            iconBg: kVioletLight,
            iconColor: kDeepViolet,
            title: 'Auto-assign complaints',
            subtitle: 'Assign to available staff automatically',
            value: _autoAssign,
            onChanged: (v) {
          ...
          _TapRow(
            ...
            subtitle: 'Manage agent access levels',
            trailing: const Icon(Icons.chevron_right_rounded,
                color: kInkLight, size: 20),
            onTap: () => _snack('Staff roles coming soon'),
          ),
        ],
      ),
    );
      ...
      child: Column(
        children: [
          _ToggleRow(
            ...
            subtitle: 'Adds extra login security',
            value: _twoFactor,
            onChanged: (v) {
          ...
          _TapRow(
            ...
            trailing: const Icon(Icons.chevron_right_rounded,
                color: kInkLight, size: 20),
            onTap: () => _snack('Login activity coming soon'),
          ),
        ],
      ),
    );
      ...
      child: Column(
        children: [
          ...
          _TapRow(
            ...
            trailing: const Icon(Icons.chevron_right_rounded,
                color: kInkLight, size: 20),
            onTap: () => _snack('Audit log coming soon'),
          ),
          const _RowDivider(),
          _TapRow(
  ...
  Widget _buildDangerCard() {
    return Container(
      ...
      child: _TapRow(
        ...
        subtitle: 'Permanently wipe all complaint records',
        titleColor: kRedDark,
        trailing:
            const Icon(Icons.chevron_right_rounded, color: kRedDark, size: 20),
        onTap: () async {
          final ok = await _confirmSheet(
            icon:         Icons.warning_amber_rounded,
            iconColor:    kRedDark,
            ...
            subtitle:
                'This will permanently delete ALL complaints, categories, and logs. This action cannot be undone.',
            confirmLabel: 'Reset everything',
            confirmColor: kRed,
          );
          if (ok && mounted) await _resetPortalData();
        },
      ),
    );
  }
...
// ══════════════════════════════════════════════════════════════════════════════
// HEADER BACKGROUND  (extracted for readability)
// ══════════════════════════════════════════════════════════════════════════════
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(99),
                          border: Border.all(
                              color: kCyan.withAlpha(80), width: 0.8),
                        ),
                        child: const Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
...
/// Individual stat chip in the header
...
/// Row with toggle switch — supports optional badge label
...
/// Tappable row with icon + arbitrary trailing widget
...
/// Rounded icon box used in every row
